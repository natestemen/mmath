...
This can be formalized using the graph structure as follows. % TODO better lead in

\begin{definition}\label{def:topology}
    A \emph{quantum connectivity graph} is simply a graph $H = (V, E)$\footnote{$V$ is a finite set which we refer to as vertices, and $E$ is a collection of pairs of vertices, \ie{} $E \subseteq V \times V$.} such that $(v, v) \in E$ for all $v \in V$.
    That is $H$ contains all self loops.
\end{definition}
The notion of a connectivity graph is sometimes referred to as a (network) topology.
In this definition we ensure a connectivity graph has all self loops as edges are the model for performing quantum gates.
Since all quantum hardware can perform single qubit unitary gates we include self edges to indicate this.
Once the connectivity graph has been established we can consider which gate-sets are possible by ensuring only qubits which are connected via an edge are acted on in a nontrivial manner.
\begin{definition}
...
\begin{question}
    ...
    Is there a circuit $C' \in G'$ such that $\dbrackets{C'} = \dbrackets{C}$?
\end{question}
