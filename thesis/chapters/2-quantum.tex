...
\begin{definition}
    ...
    \begin{equation}
        \U{n} \defeq \qty{ A \in \mats{n}{\C} : AA^\dagger = \1 = A^\dagger A} % TODO introduct matrix set notation
    \end{equation}
\end{definition}
...
Hence we define the \textbf{projective unitary group} to remove these pesky complex phases by modding out unit complex numbers: $\gls{pun} \defeq \U{n} / \U{1}$. % TODO full definition and lead in
...
\begin{itemize}
    ...
    \item \CNOT{}, Hadamard, and the $\frac{\pi}{8}$-gate\footnote{The $\frac{\pi}{8}$-gate is also sometimes called $T$ and has matrix representation $\smqty[1 & 0 \\ 0 & \e^{\iu\pi/4}]$.} as shown in~\cite{universal-cnot-had-p8} % TODO put gates in table
...
\end{equation}
where the integral $\int\dd{\rho}$ is taken over all density operators.\todo{is this even right?} % TODO replace all this with process fidelity
